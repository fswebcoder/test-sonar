...
@Component({
  selector: "svi-material-receptions-dump",
  imports: [
    TableComponent,
    PaginatorComponent,
    FloatInputComponent,
  ...
  templateUrl: "./material-receptions-dump.component.html",
  styleUrl: "./material-receptions-dump.component.scss"
})
export class MaterialReceptionsDumpComponent implements OnInit {
  materialReceptions = input.required<IOrderEntity[]>();
  storageZones = input.required<IIdName[]>();
  title = input<string>("Recepciones de material");
  emptyStateMessage = input<string>("No se encontraron recepciones de material.");

  addWeight = output<IOrderEntity>();
  registerWeight = output<IWeightRegisterParams>();

  materialReceptionModal = signal<boolean>(false);
  ...
  path = signal<string>("bascula/recepcion-de-material");
  ...
  materialReceptionList = computed(() => this.materialReceptions());
  ...
  private readonly addWeightAction: TableAction = {
    ...
    permission: {
      action: ScaleActions.REGISTRAR_PESO_RECEPCION_DE_MATERIAL,
      path: this.path()
    }
  };
  ...
  private readonly viewDetailsAction: TableAction = {
    ...
    permission: {
      action: ScaleActions.VER_RECEPCION_DE_MATERIAL,
      path: this.path()
    }
  };
  ...
  onAddWeight(order: IOrderEntity): void {
    this.materialReceptionModal.set(true);
    this.selectedOrder.set(order);
    this.dialogHeader.set(`Agregar peso a la recepción ${this.selectedOrder()!.consecutive}`);
  }

  onViewDetails(order: IOrderEntity): void {
    this.selectedOrder.set(order);
    this.detailDialogHeader.set(`Detalle de la recepción ${order.consecutive}`);
    this.detailModal.set(true);
  }

  closeMaterialReceptionModal(): void {
    this.materialReceptionModal.set(false);
  ...
  askForRegisterWeight(event: Omit<IWeightRegisterParams, "id">): void {
    this.closeMaterialReceptionModal();
    ...
    this.confirmDialog.show(
      `¿Estás seguro de registrar el peso leído para esta recepción de material con ${event.weight} ${EWeightUnits.KILOGRAMS}?`,
